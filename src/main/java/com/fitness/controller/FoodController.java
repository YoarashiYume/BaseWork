...
public class FoodController {
    ...
    @GetMapping("{id:[\\d]+}")
    public Food foodOnId(@PathVariable Long id) {
    ...
    public List<Food> getByParam(@RequestParam("id") String id,//Can`t get request with Long
    ...
    @PostMapping
    public void addFood(@RequestBody Map<String, String> body) {
        ...
        Food food = new Food();
        body.computeIfPresent("dataType", (k, v) -> {
            food.setDataType(v);
            return v;
        });
        body.computeIfPresent("desc", (k, v) -> {
            food.setDescription(v);
            return v;
        });
        body.computeIfPresent("foodCategoryId", (k, v) -> {
            food.setFoodCategoryId(pOnL(v));
            return v;
        });
    ...
    public void removeFood(@PathVariable Long id) {
        Optional<Food> result = foodRepository.findAll().stream().filter(el -> id.equals(el.getId())).findFirst();
        result.ifPresent(foodRepository::delete);
    }

    @PutMapping("{id:[\\d]+}")
    public void updateFood(@PathVariable Long id, @RequestBody Map<String, String> body) {
        ...
        body.computeIfPresent("foodCategoryId", (k, v) -> {
            newVersion.setFoodCategoryId(pOnL(v));
            return v;
        });
